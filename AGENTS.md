...
## Global Rules — read the board, not this file

Standing, cross-project rules (worktree discipline, process naming, git conventions,
agent safety, environment safety, "boil the ocean," context-management habits, and
anything else that used to be copy-pasted verbatim into every repo's `AGENTS.md`) now
live on the **keel global notice board** — one place, editable without touching or
regenerating every repo it applies to.

**At session start, read it:**
```bash
keel notice list --global
```

**A new standing rule goes to the board, not this file:**
```bash
keel notice add --global "<rule text>" --tier must|should|could|wont
```

**The board wins.** If a board notice conflicts with anything below or elsewhere in this
project, the board is the source of truth — this file carries only what is genuinely
specific to this repo (its stack, its conventions, its hooks).
## Do Not
